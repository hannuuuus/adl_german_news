    ...
    rules = (
        Rule(
            LinkExtractor(
                ...
                deny=(
                    r"www\.n-tv\.de\/mediathek\/videos\/\w.*",
    ...
    def parse_item(self, response):
        ...
        paragraphs = [
            ...
            for node in response.xpath(
                '//div/p[not(contains(@class, "article__source")) and not(descendant::strong)]'
            )
        ]
        ...
        author_person = response.xpath(
            '//div//span[@class="article__author"]/text()'
        ).getall()
        if author_person:
            author_person = [
                author.strip().split("Von ")[-1].rsplit(",")[0]
            ...
            if author_person:
                ...
                author_person = response.xpath(
                    '//div//span[@class="article__author"]/a/text()'
                ).getall()
                if author_person:
                    author_person = [
                        author.split("von ")[-1].split(",")[0]
                        for author in author_person
                    ]
                    author_person = [
                        author.split("Von ")[-1].split(",")[0]
        ...
        author_organization = response.xpath(
            '//p[@class="article__source"]/text()'
        ).get()
        item["author_organization"] = (
            author_organization.split("Quelle: ")[-1].split(", ")
        ...
        if response.xpath("//h2"):
            ...
            text = [
                ...
                for node in response.xpath(
                    '//div/p[not(contains(@class, "article__source")) and not(descendant::strong)] | //h2'
                )
            ]
