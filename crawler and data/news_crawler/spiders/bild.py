...
class BildSpider(BaseSpider):
    ...
    rules = (
        Rule(
            LinkExtractor(
                ...
                deny=(
                    r"www\.bild\.de\/\w+\/international\/\w.*\.bild\.html$",
    ...
    def parse_item(self, response):
        ...
        paragraphs = [
            ...
            for node in response.xpath('//div[@class="txt" or @class="article-body"]/p')
        ]
        ...
        author_person = response.xpath(
            '//div[@class="authors"]//span[@class="authors__name"]/text()'
        ).get()
        if author_person:
            author_person = (
                author_person.split(" UND ")
                if "UND" in author_person
                else [author_person]
            )
        else:
            author_person = response.xpath(
                '//div[@class="author"]//span[@class="author__name"]/text()'
            ).get()
        ...
        if response.xpath('//h2[@class="crossheading"]'):
            ...
            headlines = [
                ...
                for h2 in response.xpath('//h2[@class="crossheading"]')
            ]

            text = [
                node.xpath("string()").get().strip()
                for node in response.xpath(
                    '//div[@class="txt" or @class="article-body"]/p | //h2[@class="crossheading"]'
                )
            ]
        ...
        recommendations = response.xpath(
            '//div[@class="related-topics__container"]/article/a/@href'
        ).getall()
        if not recommendations:
            recommendations = response.xpath(
                '//div[descendant::h3[contains(text(), "Lesen Sie auch")]]/ul/li//a/@href'
            ).getall()
